...
root.minsize(width=700,height=500)#make default size
...
def srch(event):
    ...
    txt_srch.insert(END,food.print_table(listresult))
    txt_ind.delete("1.0",END)
    txt_ind.insert(END,food.print_index(listresult))

#show results after sorting by rank
...
#show results after sorting by price
...
class food:
    ...
    def search(thislist,keyword):
        textlist = []
        newlist = [[food.convert(b) for b in a] for a in thislist]
    ...
    #tabulate format
    def print_index (thislist):
        x=0
        abc="\n\n"
        for thislist[x] in thislist:
            abc += str(x+1)
            abc += "\n"
            x+=1
        return abc
    def print_table (thislist):
        header = ["foodcourt","category","rating","price"]
        return tabulate(thislist,header,tablefmt=".4f")
...
txt_ind = Text(bottomFrame,height=24,width=5)
txt_ind.pack(side=LEFT)

txt_srch = Text(bottomFrame,height=24,width=70)
txt_srch.pack(side=LEFT)

button_qt = Button(quitFrame,text="Quit")
button_qt.pack(side=BOTTOM)
